...
I will conduct generate three linear models exploring  participants’ support for resettling refugees as the dependent measure and their kind-of-person mind-sets, mind-sets about intelligence, and political orientation as independent variables. These three models will be compared using an ANOVA with the key statistic being the p-value showing that the third model (which includes the mindset measure) differs from the models that do not include that measure. This value is reported to be <.01 in the original paper.
...
As done in the original study, I plan to build three linear models. The first one will look at how mindsets about intelligence predict support for refugees. The next model will add political orientation to the independent variables and the third model will add kind of person mindsets. I will also run another regression examining the effect the order of the survey measures (support for resettling refugees measure first or mindsets measures first) and the interaction between kind of person mindsets and presentation order have on participant's support for resettling refugees. These four models will then be compared using a four-way ANOVA. The key statistic will be examining whether the third model (which includes the kind of person mindset measure) differs significantly from the models that do not include that measure.
